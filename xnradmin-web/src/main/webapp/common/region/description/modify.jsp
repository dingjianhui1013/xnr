...
<%
	...
	String getCbm = basePath+"page/Common/Region/Description/all.action";
	String actionForm = basePath+"page/Common/Region/Description/modify.action";
	...
	String communityLookup = basePath + "page/Common/Region/Community/lookup.action";
	...
	request.setAttribute("communityLookup",communityLookup);
...
<div class="pageContent">
	<form method="post" action="${actionForm}" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
		<div class="pageFormContent" layoutH="56">
			...
			<p>
				<label>社区：</label>
				<input id="community_id" class="readonly" name="communitytype.id" readonly="readonly" value="${communityid}" type="hidden"/>
				<input readonly="readonly" name="communitytype.community" value="${communityname}" type="text"/>	
				<a class="btnLook" href="${communityLookup}?provinceid={province_id}&cityid={city_id}&areaid={area_id}&roadid={road_id}" lookupGroup="communitytype">查找带回</a>	
			</p>
			<p>
				<label>具体描述：</label>
				<input id="description_id" name="descriptionid" readonly="readonly" value="${descriptionid}" type="hidden"/>
				<input name="description" type="text" size="30" value="${description}" class="required"/>
			</p>
		</div>
		<div class="formBar">
			<ul>				
