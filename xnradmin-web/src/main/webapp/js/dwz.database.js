			...
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
						...
						mobile: $th.attr("mobile") || "",
						descriptionid: $th.attr("descriptionid") || "",
						readonly: $th.attr("readonly") || "",
						description: $th.attr("description") || ""
					};
					fields.push(field);
				});
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					case 'lookup':
						...
						if(field.mobile){
							html = html	+ '<input type="hidden" name="ruleList[#index#].agentCurrenStaff.'+field.mobile+suffix+'"/>';
						}
						if(field.description){
							html = html	+ '<input type="hidden" name="ruleList[#index#].provinceVO.'+field.description+suffix+'"/>';
							
						}
						
						html = html	+ '<input type="text" name="'+field.name+'"'+suggestFrag+' lookupPk="'+field.lookupPk+'" size="'+field.size+'" class="'+field.fieldClass;
						if(field.readonly!=null && field.readonly.length>0) 
							html += '" readonly="' + field.readonly;
						html = html + '"/>';
						
						html = html+ '<a class="btnLook" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" title="查找带回">查找带回</a>';
						break;
					case 'attach':
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
