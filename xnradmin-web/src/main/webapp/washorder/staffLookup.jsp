...
<%
	...
	String action = basePath+"page/order/staffLookup.action";	
	...
	<table class="table" layoutH="118" targetType="dialog" width="100%">
		...
		<tbody>
			<c:if test="${staffList!=null}">
				<c:forEach items="${staffList}" var="loop">
					<tr>								
						<td>${loop.userid}</td>
						<td>${loop.staffName}</td>	
						<td>${loop.statusName}</td>											
						<td>
							<a class="btnSelect" href="javascript:$.bringBack({userid:'${loop.userid}',staffName:'${loop.staffName}',mobile:'${loop.mobile}'})" title="查找带回">选择</a>
						</td>							
					</tr>				
				</c:forEach>
			</c:if>			
