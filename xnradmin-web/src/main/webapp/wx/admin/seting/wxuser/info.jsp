...
<%
	...
	String action = basePath + "page/wx/wxuser/info.action";	
	...
	String wxmenusync = basePath+"page/wx/wxmenu/reload.action";
	...
	request.setAttribute("wxmenusync", wxmenusync);
%>
					...
					<tr>						
						...
						<td>				
							...
							<a title="设置为删除状态?" target="ajaxTodo" href="${del}?query.wxuser.id=${loop.wxuser.id}&del=1" class="btnDel">删除</a>
							 -->	
							<a title="编辑" rel="portModify" target="navTab" href="${modify}?query.wxuser.id=${loop.wxuser.id}" class="btnEdit">编辑</a>
							<a title="查看详情" rel="viewWxUserDetail" target="dialog" href="${view}?query.wxuser.id=${loop.wxuser.id}" class="btnLook">查看</a>
							<a title="重新加载accessToken?" target="ajaxTodo" href="${reload}?query.wxuser.id=${loop.wxuser.id}" class="btnRefresh">重新加载accessToken</a>
							<a title="同步该用户的菜单?" target="ajaxTodo" href="${wxmenusync}?query.wxuservo.wxuser.id=${loop.wxuser.id}" class="btnSelect">同步该用户的菜单</a>
						</td>	
					</tr>
				</c:forEach>
			</c:if>		
