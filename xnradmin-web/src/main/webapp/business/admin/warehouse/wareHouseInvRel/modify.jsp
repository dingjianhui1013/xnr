...
<div class="pageContent">
	...
	<form method="post" action="${actionForm}" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
		...
		<input name="wareHouseInvRelId" value="${wareHouseInvRelId}" type="hidden"/>
		<fieldset>
				<label>选择仓库：</label>
				<select class="combox" name="wareHouseInvRelWareHouseId">
					<c:if test="${businessWareHouseList!=null}">
						<c:forEach items="${businessWareHouseList}" var="loop">
							<c:choose>
								<c:when test="${loop.id==businessWareHouseVO.businessWareHouseInvRel.wareHouseId}">
			...
			<fieldset>
				<label>选择商品：</label>
				<input class="readonly" name="goods.id" readonly="readonly" value="${goods.id}" type="hidden"/>
			...
			<fieldset>
				<label>商品数量：</label>
				<input name="wareHouseInvRelCount" type="text" size="25" value="${businessWareHouseVO.businessWareHouseInvRel.count}" class="required number"/>
			</fieldset>
		</div>
		<div class="formBar">
			<ul>				
