...
<%
	...
	String action = basePath+"page/menu/selectLookup.action";
	...
	<table class="table" layoutH="118" targetType="dialog" width="100%">
		<thead>
			<tr>
				...
				<th width="55" orderField="menu_level" class="${menu_level}">菜单级别</th>				
				<th width="80">查找带回</th>
			</tr>
		</thead>
		<tbody>
			<c:if test="${voList!=null}">
				<c:forEach items="${voList}" var="loop">
					<tr>
						<td><input type="checkbox" name="orgId" value="{id:'${loop.id}', menuName:'${loop.menuname}', orgNum:'${loop.id}'}"/></td>
						<td>${loop.menuname}</td>
						<td>${loop.id}</td>
