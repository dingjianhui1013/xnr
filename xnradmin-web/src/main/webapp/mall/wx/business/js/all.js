...
(function(){
  ...
  window.clearCache = function(){
    ...
    var _key = _.find(_.keys(localStorage), function(key){ return /^\d+$/.test(key);});
    if(_key){
  ...
  PlusMinus = {
    ...
    detailHTMLTemple: function(item, hasFavorite, needDelete)
    {
      ...
      var cateBlock = "";
      cateBlock = "<span class='js-subject' style='color:#666;'>分类："+item["class2"].trim()+"</span></br/>";
      //subject
      var subjectBlock = "";
      ...
      var prePrice = "";
      var soldRef = '';
      //var soldRef = '<div class="ml10" style="height:12px;line-height:12px;font-size:12px;color:#666;">' + 
       //   '已售'+ item["sold_count"] + (item["unit"] || "").substring(0,1) + '</div> ';
     ...
     // }
      ...
      if(item['standard_item_num'] && item['standard_item_num'] > 1){
        ...
        //打包售卖模型
        formatRef = '<span class="js-format">(' + item["format"] + ')</span>';
      }
      ...
      //td2
      var modifyCountHTML = ' ' +
         '<a class="js-minus cp font20" style="color:#ffbc57;font-weight:bold;border: 1px solid;">－</a>' +
         ...
         ' <a class="js-plus cp font20" style="color:#ffbc57;font-weight:bold;border: 1px solid;">＋</a>';
      ...
      if(item["outOfDate"]){
       ...
       var td2HTML = soldRef 
       +'<div class="ml10 product-img-wrapper">'
       +'<img class="" data-srcoriginal="http://img.yunshanmeicai.com/fa6d963a8e22.png" data-src="http://img.yunshanmeicai.com/fa6d963a8e22_thumb.png" src="http://img.yunshanmeicai.com/fa6d963a8e22_thumb.png">'
       +'</div>'
      ...
      */
      ...
      var tmplHTML =
        ...
        '  ' + '<span class="js-c-name">' + item["name"]  + levelBlock + favoriteBlock + '<br />' + 
        '</span>'+cateBlock+subjectBlock  + 
            td1Mask  + 
        ' ' + prePrice + '<span class="js-price price">' + finalPrice +'</span>元/' + formatRef + priceRef + 
        '  </td>' +
    ...
    allJson:function(){
    	...
    	for(var key in localStorage){
          ...
          if(key !== USERKEY && /^\d+$/i.test(key)){
          	...
          	//var item = eval(localStorage[key]);
          	var temp = localStorage[key];
          	res+=temp;
          	//console.log("flag:"+flag+" | length : "+localStorage.length);
          	if(flag!=localStorage.length){
          		res+=",";
          	}
        ...
        $.ajax({
				type: 'GET',
				url:"/page/wx/client/web/categorylist/categoryList.action",
				data:res,
				dataType:"json",
				cache: false,
				...
				success:function(result) {  
					//console.log("categoryHeadLogo: "+result.categoryList[0].categoryHeadLogo);
		            for(var i in result) {  
    ...
    detailJson:function(item){
	      ...
	      var favoriteBlock = "";
	      if(hasFavorite){
	      	favoriteBlock = item['is_favorite'];
	      }

	      //subject
	      var subjectBlock = "";
	      if(item["subject"] && item["subject"].trim() !== ""){
	        subjectBlock = item["subject"].trim();
	      }
	      //price, final priceref
	      var finalPrice = item['commodity_total_price'] ? item['commodity_total_price'] : item["price"];
	      var priceRef = "";
	      ...
	      //console.log("favoriteBlock:"+favoriteBlock);
    ...
    bindPlusMinusEvent: function (tr,options){
      ...
      $tr.find(".js-plus").click(function(){
        ...
        var catName = $tr.find(".js-subject").text();
        var parentCatName = $tr.find(".js-subject").text();
        /**
        if(catName.indexOf("糖盐")>-1){
        	alert("此类商品，如果您在当日下午14：30之前订购，将于次日配送到达，如果于当日14：30之后订购，将于次次日送达");
        }
        */
    ...
    bindToggleFavorite: function($list, hasFavorite, isPreview){
      ...
      _.each($list.find("tr td:nth-child(1)"), function(td){
        ...
        $td.click(function(){
          $this = $(this);
          var s_id = JSON.parse(localStorage[USERKEY]).id;
          ...
          var data = {
            goodsId : c_id, staffId : s_id
          };

          var $mask = $this.find(".js-td1-mask");
          if(!$mask.hasClass('hide')){$mask.addClass('hide'); return;}
          if(action == 'cancel'){
          ...
          var postUrl = '';
          if(isPreview){
            postUrl = '/page/wx/client/web/business/userfavorite/add.action?type='+action;
          }
    ...
    storeData: function ($tr){
      ...
      if(_count > 0){
        localStorage[primary_key] = JSON.stringify({id: primary_key, count: _count, price: _price, is_favorite: 1});
      }else{
