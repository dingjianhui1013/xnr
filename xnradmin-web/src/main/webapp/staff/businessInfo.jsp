...
<%
	...
	String action = basePath + "page/staff/businessInfo.action?queryOrgid=15";	
			...
			<c:if test="${voList!=null}">
				<c:forEach items="${voList}" var="loop">
					...
					<tr target="sid_queryid" rel="${loop.userid}">
						...
						<td>				
							<a title="增加订单" target="navTab" href="${addOrder}?staffId=${loop.userid}&divid=statBusinessOrderInfo" class="btnAdd">增加订单</a>
							<a title="编辑" rel="businessStaffInfo" target="navTab" href="${modify}?queryid=${loop.userid}&queryOrg1.id=${org1.id}&queryOrgid=15&queryOrg1.organizationName=${org1.organizationName}&queryStaffname=${staffname}&queryStime=${sTime}&queryEtime=${eTime}&queryStaffStatus=${staffStatus}&totalCount=${totalCount}&pageNum=${pageNum}&numPerPage=${numPerPage}&orderField=${orderField}&orderDirection=${orderDirection}&divid=statBusinessOrderInfo" class="btnEdit">编辑</a>
							<c:if test="${loop.statusid==7}">
								<a title="确认审核通过?" target="ajaxTodo" href="${modifyStatus}?queryid=${loop.userid}&queryOrg1.id=${org1.id}&queryOrgid=15&queryOrg1.id=${org1.id}&queryOrg1.organizationName=${org1.organizationName}&queryStaffname=${staffname}&queryStime=${sTime}&queryEtime=${eTime}&totalCount=${totalCount}&pageNum=${pageNum}&numPerPage=${numPerPage}&orderField=${orderField}&orderDirection=${orderDirection}&queryStaffStatus=1&divid=statBusinessOrderInfo" class="btnSelect">审核通过</a>
							</c:if>
							<!--
