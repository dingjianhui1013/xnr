...
public class ZipHelper {
	...
	public static void compress(File srcFile, File destFile) throws IOException {
		...
		compress(srcFile, destFile, zos, BASE_DIR);
	 ...
	 * @throws Exception
	 */
	private static void compress(File srcFile, File destFile,
			ZipOutputStream zos, String basePath) throws IOException {
		if (srcFile.isDirectory()) {
			compressDir(srcFile, destFile, zos, basePath);
		} else {
	 ...
	 * @throws Exception
	 */
	private static void compressDir(File dir, File destFile,
			ZipOutputStream zos, String basePath) throws IOException {
		...
		for (File e : files) {
			if (e.getName().equals(destFile.getName()))
				continue;
			l.add(e);
		}
		...
		for (int i = 0; i < l.size(); i++) {
			// 递归压缩
			compress(l.get(i), destFile, zos, basePath + dir.getName() + PATH);
		}
	}
	...
	public static void unCompress(String zipFile, String outPath)
		...
		if (!outPath.substring(outPath.length() - 1, outPath.length()).equals(
				"/"))
			outPath += "/";
