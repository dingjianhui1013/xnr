...
public class QueryExample{
	private static Logger log = Logger.getLogger(QueryExample.class);
	public static String postQE(String orderNumber){
        // 请求要素
		...
		req.put("orderTime", new SimpleDateFormat("yyyyMMdd").format(new Date()));// 交易开始日期时间yyyyMMddHHmmss或yyyyMMdd
		...
		Map validResp = UpmpService.query(req, resp);
		if(validResp !=null && validResp.get("flag")!=null && validResp.get("tn")!=null){
			String flag = validResp.get("flag").toString();
			String tn = validResp.get("flag").toString();
			 // 商户的业务逻辑
